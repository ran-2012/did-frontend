...
function ModalTestGround() {
    ...
    const showQrModal = useMyModal(QrCodeModal);

    useEffect(() => {
        console.log("init modal test ground")

        const e = crypto.encrypt('123');
        console.log('encrypted: ' + e);
        console.log('decrypted: ' + crypto.decrypt(e));
    }, []);

    useEffect(() => {
        console.log('showQrModal: ' + showQrModal);
        if (!showQrModal) return;
        showQrModal.show({qrString: '1231231231231321554635413654135'})
    }, [showQrModal]);
